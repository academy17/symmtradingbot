...
const { sendQuoteWithAffiliateFunctionAbi } = require('./abi/SendQuoteWithAffiliate');
...
async function executeSendQuoteWithAffiliate(
    subAccountAddress, 
    positionType, 
    orderType, 
    quantity, 
    ...
    if (signatureResult.success) {
      ...
      * (new BigNumber(lockedParams.partyAmm * 100))
      ...
      / (new BigNumber(lockedParams.leverage))
      / (new BigNumber(1e18));
    console.log("partyAmmWei: ", partyAmmWei);
    ...
    const encodedSendQuoteData = web3.eth.abi.encodeFunctionCall(sendQuoteWithAffiliateFunctionAbi, sendQuoteParameters);
  ...
  async function run() {
    ...
    const affiliate = '0x141269E29a770644C34e05B127AB621511f20109'; //IntentX Affiliate
    const maxFundingRate = web3.utils.toWei('200', 'ether'); //Max Funding Rate
        try {
            ...
            const quoteId = await executeSendQuoteWithAffiliate(
              subAccountAddress, 
              0, // LONG
              1, 
              config.QUANTITY, 
